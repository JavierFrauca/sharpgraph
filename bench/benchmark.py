#!/usr/bin/env python
"""
Benchmark de tokens a 3 bandas — SharpGraph vs CodeGraph vs sin-MCP — sobre los
repos y preguntas que definas en un módulo de preguntas.

Uso:  python benchmark.py [ruta_exe_sharpgraph] [modulo_questions]
...
for a in _args:
    ...
    elif a.endswith(".exe") or a == "SharpGraph" or os.path.isfile(a):
        EXE = a
if EXE is None:
    EXE = os.path.join(os.path.dirname(__file__), "..", "publish", "SharpGraph.exe")
    if not os.path.isfile(EXE):
        EXE = os.path.join(os.path.dirname(__file__), "..", "src", "SharpGraph",
                           "bin", "Release", "net10.0", "win-x64", "SharpGraph.exe")
...
try:
    ...
    print("  python benchmark.py publish/SharpGraph.exe questions.cleanarchitecture.py")
    sys.exit(1)
...
_DEFAULT_EXE = os.path.join(os.path.dirname(__file__), "..", "src", "SharpGraph",
                            "bin", "Release", "net10.0", "win-x64", "SharpGraph.exe")
...
# ---------------- SharpGraph por stdio ----------------
def run_sharpgraph(repo, calls):
...
def main():
    ...
    for repo_key, calls in by_repo.items():
        print(f"SharpGraph: indexando y consultando {repo_key} ({len(calls)} llamadas)...")
        lg_results.update(run_sharpgraph(REPOS[repo_key], calls))
    ...
    for q in Q:
        ...
        elig = {}
        if not literal and lg_ok: elig["SharpGraph"]=lg_tok
...
def write_report(rows):
    o=[]
    o.append(f"# Benchmark de tokens — SharpGraph vs CodeGraph vs sin-MCP ({len(rows)} preguntas, 2 repos)\n")
    ...
    wins={"SharpGraph":0,"CodeGraph":0,"sin-MCP":0}
    for r in rows:
    ...
    o.append(f"- **Total tokens ({len(rows)} preguntas)** — SharpGraph: **{tl}** · CodeGraph: **{tc}** ({tc/tl:.1f}×) · sin-MCP: **{tn}** ({tn/tl:.1f}×)")
    o.append(f"- **Preguntas ganadas (menos tokens)** — SharpGraph: **{wins['SharpGraph']}/{len(rows)}** · CodeGraph: **{wins['CodeGraph']}/{len(rows)}** · sin-MCP: **{wins['sin-MCP']}/{len(rows)}**")
    ...
    o.append(f"  - **Navegar / localizar / resolver** ({len(nav)} preguntas, el uso dominante de un agente): "
             f"SharpGraph **{na}** tok vs CodeGraph **{nb}** ({nb/na:.1f}×) vs sin-MCP **{nc}** ({nc/na:.1f}×). Aquí SharpGraph arrasa.")
    ...
    o.append("| favor | n | SharpGraph | CodeGraph | sin-MCP | gana |")
    o.append("|---|---|---|---|---|---|")
    for fav in ["LG","CG","FLOW","GREP","NEU"]:
        ...
        w={"SharpGraph":0,"CodeGraph":0,"sin-MCP":0}
        for r in rs:
    ...
    for r in rows:
        ...
        o.append(f"| {q['id']} | {q['repo']} | {q['favor']} | {q['q']} | "
                 f"{mark('SharpGraph',r['lg'],r['lg_ok'] and not r['literal'])} | "
    ...
    o.append(f"- **Estructura / navegación .NET** (favor LG): SharpGraph gana la mayoría (12/17). Responde con metadatos "
             ...
             f"`search`+`find_callers` con árbol de profundidad 3 de SharpGraph. Si SharpGraph usara `depth:1` empataría; "
    ...
    o.append(f"- **Comprensión de FLUJO** (favor FLOW): la nueva tool `flow` destila el árbol de llamadas (siguiendo "
             ...
             f"{next((r['nm'] for r in rows if r['q']['id']=='F1'),0)} (grep) o {next((r['cg'] for r in rows if r['q']['id']=='F1'),0)} (explore). **Aquí SharpGraph también "
    ...
    o.append(f"- **Literales** (favor GREP): grep gana 3/3. Los grafos **no indexan literales** — SharpGraph y CodeGraph "
             f"directamente no responden (†). Para 'dónde está este string', grep es la herramienta correcta.")
    o.append(f"- **Honestidad**: en R06 SharpGraph **falló** (no encontró los call-sites de `RunInitialIndexAsync`, †) y ganó "
             f"CodeGraph. El benchmark mide coste, no calidad: aun así SharpGraph aporta relación/línea/binding/centralidad y "
    ...
    o.append(f"- **Veredicto**: para el uso dominante de un agente —navegar, localizar, resolver DI, trazar a endpoints— "
             f"SharpGraph ahorra **~4-6×** tokens. CodeGraph brilla cuando el objetivo es *leer/comprender* código a fondo. "
             f"grep, para literales. No hay un único ganador universal, pero en .NET y para minimizar tokens, SharpGraph "
             f"gana el grueso de las consultas reales.\n")

    o.append("## Metodología\n")
    o.append("- **SharpGraph**: salida real de sus herramientas MCP (exe dirigido por stdio, un índice por repo).")
    ...
    o.append("- **sin-MCP**: salida de `grep` + contenido íntegro de los ficheros con hit, **excluyendo tests/samples** "
             "(a favor de sin-MCP; SharpGraph los filtra solo).")
    o.append("- Se mide coste, no calidad. SharpGraph además aporta relación/línea/binding y filtra tests; CodeGraph mezcla "
    ...
    print(f"\nLG={tl}  CG={tc}  NM={tn}  | wins LG/CG/NM = {wins['SharpGraph']}/{wins['CodeGraph']}/{wins['sin-MCP']}")
